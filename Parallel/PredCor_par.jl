...
function pred_cor_global(A, eps, k)
    ...
    Z = fill(0.0 + 1.0im, k)
    Threads.@threads for i in 0 : length(egv)-1
...
function plots(A, eps, k)
    ...
    title!(string("Pred-Cor : Pseudospectra of Matrix A, ε = ", string(eps)))
    xaxis!("Re")
    yaxis!("Im")
...
function plots(A, eps, k)
    ...
    for i in 1 : length(egv) 
        Re_Z = real(Z[i, :])
        Im_Z = imag(Z[i, :])
        data = Plots.plot!(Re_Z, Im_Z, labels = ["pseudospectra" ""], color ="black")
        plotting = plot!(data, fmt= :png) 
    end
    scatter!(eigvals(A), lab="eigenvalues", color="red")
