...
"""
Generating a complex matrix of dimension n.
"""
...
"""
function comp_compute(A, eps)
    (X, Y) = generate_grid(A, eps)
	comp = zeros(length(X), length(Y))
    E = fill(1, size(A, 1), size(A, 2))
	for i in 1:100
		for j in 1:100
            res = norm(inv(A - (X[i] + Y[j] * 1im)*I)) * E
            res = maximum(broadcast(abs, eigvals(res)))
			comp[j,i] = 1/res
		end
	end
	return comp, X, Y
...
function contours(A, eps)
    ...
    xaxis!("Re")
    yaxis!("Im")
    filename = string("contour_comp", string(eps),".png")
...
"""
function execute(A)
    pswindow = GtkWindow("Componentwise pseudospectra", 400, 100)
    grid = GtkGrid()
